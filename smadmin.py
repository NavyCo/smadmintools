...
def get_plugin_name_and_link(b: BeautifulSoup):
    ...
    # Fieldset is the tag for the attachment table.
        ...
        print(
            "Error: Could not find a fieldset tag in data. Either the HTML layout has changed or the thread was entered incorrectly.")
    ...
    for table in tables:
        ...
        if 'Get Plugin' in table.text:
            ...
            if maindl is None:
                ...
                other_smxs.append(
                    (table.find_all('a')[0].get('href'), table.text.split('(')[1].split('.sp')[0] + '.smx'))
        else:
            if '.smx' in table.text:
                # Extra plugin SMX
                other_smxs.append(
                    (table.find_all('a')[0].get('href'), table.text.replace('\n', '').split('(')[0][:-1]))  # Kill me
            else:
...
def install_file(flink, ftype='', name="guess"):
    ...
    if ftype == 'smx':
        ...
        if '.smx' in flink:
            ...
            if '/' not in name:
                print(
                    "Unrecognised file type. Cannot install properly. Please provide a directory along with the name.")
                return
            else:
    ...
    if not "http://" in flink and not "https://" in flink:
        ...
        if '.php' in flink and name == "guess":
            ...
            if r.status_code != 200:
                ...
                return False
            if name == "guess": name = flink.split('/')[-1].split('?')[
                0]  # Get the last part of the name without any args or such
...
def get_user_input_plugin_url(url):
    ...
    if other_smxs == []:
        ...
        if maindl is None:
            print(
                "Warning: No SMX files identified. This may be a zip-plugin (results may vary) or a data-only plugin.")
